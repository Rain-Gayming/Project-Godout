...
enum SamplingMode { Process, PhysicsProcess, None }

enum TangentAxis { X, Y, Z }
...
@export var sampling_mode: SamplingMode = SamplingMode.Process:
	set = set_sampling_mode
@export var max_points = 10:
	set = set_max_points  # (int,0,1000)
...
@export var interpolate_skip: bool = false:
	set = set_interpolate_skip
...
func push_xform(xform = null):
	...
	if xform == null:
		points[0] = null
		return
	if is_zero_approx(damping):
		damped_transform = xform
	else:
		damped_transform = xform.interpolate_with(damped_transform, damping)
	points[0] = damped_transform


func _redraw():
	if visible:
		_mesh_instance.mesh = _linegen.draw_from_xforms_strip(
			points, global_transform.inverse(), tangent_axis
		)
